<div class="space-y-4">
    @php
        $project = $getRecord();
        $progresses = $project ? $project->progresses()->with('user')->orderBy('created_at', 'desc')->get() : collect();
    @endphp
            ...
            @foreach($progresses as $progress)
                <div>
                    <div class="text-sm font-medium text-gray-900 dark:text-white flex items-center gap-2">
                        <span>{{ $progress->user?->name ?? 'Sistema' }}</span>
                        @if($progress->user?->processRole)
                            <span class="inline-flex items-center rounded-md bg-primary-50 px-2 py-1 text-xs font-medium text-primary-700 ring-1 ring-inset ring-primary-700/10 dark:bg-primary-400/10 dark:text-primary-400 dark:ring-primary-400/30">
                                {{ $progress->user->processRole->name }}
                            </span>
                        @endif
                    </div>
                    <time class="mb-1 text-xs font-normal text-gray-400">{{ $progress->created_at->format('d/m/Y \à\s H:i') }}</time>
